...
from utils.render_utils import get_video_cams, to8b
...
def render_video(model_path, name, iteration, views, gaussians, pipeline, background):
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), f"video")
    ...
    with torch.no_grad():
        ...
        for idx, view in enumerate(tqdm(views_video, desc="Rendering video progress")):
            render_pkgs.append(render(view, gaussians, pipeline, background))
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video : bool):
    with torch.no_grad():
        ...
        if not skip_video:
            render_video(dataset.model_path, "video", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background)
