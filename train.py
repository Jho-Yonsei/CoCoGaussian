...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians.create_blurring_kernel(opt, train_cams=scene.getTrainCameras().copy())
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        blurring = 0 if iteration < opt.start_blurring else 1
        
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, blurring=blurring, iteration=iteration, start_weight=opt.start_weight)
        ...
        with torch.no_grad():
            ...
            # Log and save
            if iteration == 2500:
                with open(f"{dataset.model_path}/log.txt", "a") as f:
                    f.write("[ITER {}] MAX DEPTH: {} \n".format(iteration, gaussians.CoCoBlurKernel.max_depth))
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    if config['name'] == "train":
                        render_pkg = renderFunc(viewpoint, scene.gaussians, *renderArgs, blurring=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[3_000, 6_000, 9_000, 12_000, 15_000, 18_000, 21_000, 24_000, 27_000, 30_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[3_000, 6_000, 9_000, 12_000, 15_000, 18_000, 21_000, 24_000, 27_000, 30_000])
